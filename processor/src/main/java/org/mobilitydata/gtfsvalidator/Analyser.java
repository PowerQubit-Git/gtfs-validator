...
public class Analyser {
  ...
  public GtfsFileDescriptor analyzeGtfsFileType(TypeElement type) {
    ...
    fileBuilder.setClassName(GtfsEntityClasses.entityImplementationSimpleName(type.getSimpleName().toString()));
    ...
    for (ExecutableElement method : methodsIn(type.getEnclosedElements())) {
      ...
      if (method.getAnnotation(Positive.class) != null) {
        fieldBuilder.setNumberBounds(RowParser.NumberBounds.POSITIVE);
      } else if (method.getAnnotation(NonNegative.class) != null) {
        fieldBuilder.setNumberBounds(RowParser.NumberBounds.NON_NEGATIVE);
      } else if (method.getAnnotation(NonZero.class) != null) {
        fieldBuilder.setNumberBounds(RowParser.NumberBounds.NON_ZERO);
      }
  ...
  public GtfsEnumDescriptor analyzeGtfsEnumType(TypeElement type) {
    ...
    enumBuilder.setName(EnumGenerator.createEnumName(type.getSimpleName().toString()));
